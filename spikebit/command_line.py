...
def server_main():
    ...
    # parser.add_argument("--port", help="port to connect to on server",
    ...
    for isys in range(1,args.nsys+1):
    	...
    	for itrial in range(1,args.ntrials+1):
    		...
    		hdc=hdfcom.NMHdf(filename,args.fs,ich,args.bufsz,args.nsys)
    		hdc.close()
    		
    		print("Spawning server no %d, trial %d",isys,itrial)
    		comm = MPI.COMM_SELF.Spawn(sys.executable,args=['serverwrapper.py',
    														'--fs={}'.format(args.fs),
    														"--nch={}".format(ich),
    														'--bufsz={}'.format(args.bufsz),
    														'--filename={}'.format(filename),
    														'--simsz={}'.format(args.simsz)],maxprocs=isys)
    		comm.Disconnect()

def client_main():
    """
    Right now a dummy client, but will be extended to 
    """
    parser = argparse.ArgumentParser()
    ...
    for itrial in range(1,nTrials+1):
        	try:
        		neurosim.runsim(nCh,simSz,bufSz,fs,hostname,port)
        	except socket.error:
